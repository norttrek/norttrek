...
function get_total($route,$imei){
  ...
  for($i=0;$i<count($route);$i++){
    ...
    $fuel_b = substr($route[$i]['ada_v'],4,4)/100;
    $fuel_c = substr($route[$i]['fuel'],0,4)/100;
    $data[$i]['t1'] = $objGPRS->get_fuel_lt($fsensors['fuel_a_d'],$fsensors['fuel_a_l'],$fsensors['fuel_a_as'],$fsensors['fuel_a_v'],$fsensors['fuel_a_vl'],$fuel_a);
	$data[$i]['t2'] = $objGPRS->get_fuel_lt($fsensors['fuel_b_d'],$fsensors['fuel_b_l'],$fsensors['fuel_b_as'],$fsensors['fuel_b_v'],$fsensors['fuel_b_vl'],$fuel_b);
	$data[$i]['t3'] = $objGPRS->get_fuel_lt($fsensors['fuel_c_d'],$fsensors['fuel_c_l'],$fsensors['fuel_c_as'],$fsensors['fuel_c_v'],$fsensors['fuel_c_vl'],$fuel_c);
	$data[$i]['t1'] = number_format($data[$i]['t1']+$data[$i]['t2']+$data[$i]['t3'],2);
  ...
  if($rpt_tt[$i]['detected']==1){
   if($rpt_tt[$i]['val'] > 0){ $color = '#59ad00'; }else{ $color = '#eb0000'; }
    echo '<td>
			...
			<div style="position:absolute; display:none; width:170px; height:120px; border:#000 solid 1px;top:-10px;text-indent:0; padding-left:5px; background-color:#fff; color:#666; margin-left:55px;">'.$rpt_tt[$i]['chart'].'</div>'.$rpt_tt[$i]['t1'].'
			</div>
		  </td>';
   }else{
     // aqui
