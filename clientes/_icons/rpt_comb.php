...
<style>
#report { width:99%; font-size:12px; margin:auto; border:#e4e4e4 solid 1 px; background-color:#fff;}
#report .tabs { background-color:#eeeeee; }
#report .tabs ul { margin:0; padding:0; list-style:none; }
#report .tabs ul li { float:left; border-right:#d6d6d6 solid 1px; border-bottom:#e1e1e1 solid 1px;}
#report .tabs ul li.active { background-color:#fff; border-bottom:#fff solid 1px;}
#report .tabs ul li.active a { color:#4d4d4d; display: inline-block;  } 
#report .tabs ul li a { display:inline-block; height:30px; text-align:center; text-decoration:none; padding-top:15px; color:#7f7f7f;  padding-left:45px; padding-right:45px;}
#report .tab_container { min-height:600px; }
#report .tab_container .tab { padding:15px; display:none; }
#report .tab_container .tab.active { display:block; }
.clear { clear:both;}
</style>
</head>
<?php
...
$t1 = get_t1($route,$imei);
$rpt = array_reverse($t1);

$t2 = get_t2($route,$imei);
$rpt_t2 = array_reverse($t2);

$t3 = get_t3($route,$imei);
$rpt_t3 = array_reverse($t3);

$tt = get_total($route,$imei);
...
for($i=0;$i<count($rpt);$i++){
  ...
  if($rpt[$i]['acum_lts']!=0){
	  ...
	  $buffer_t1 .= "{ y: ".$rpt[$i]['t1'].", marker: { symbol: 'url(".$icon.")' }, url: 'http://maps.google.com/?q=".$route[$i]['lat'].",".$route[$i]['lng']."', custom : '".$custom."' },"; 
  }else{
  ...
  if($rpt_t2[$i]['acum_lts']!=0){
	  ...
	  $buffer_t2 .= "{ y: ".$rpt_t2[$i]['t2'].", marker: { symbol: 'url(".$icon.")' }, url: 'http://maps.google.com/?q=".$route[$i]['lat'].",".$route[$i]['lng']."', custom : '".$custom."' },"; 
  }else{
  ...
  if($rpt_t3[$i]['acum_lts']!=0){
	  ...
	  $buffer_t3 .= "{ y: ".$rpt_t3[$i]['t3'].", marker: { symbol: 'url(".$icon.")' }, url: 'http://maps.google.com/?q=".$route[$i]['lat'].",".$route[$i]['lng']."', custom : '".$custom."' },"; 
  }else{
  ...
  if($rpt_tt[$i]['acum_lts']!=0){
	  ...
	  $buffer_tt .= "{ y: ".$rpt_tt[$i]['t3'].", marker: { symbol: 'url(".$icon.")' }, url: 'http://maps.google.com/?q=".$route[$i]['lat'].",".$route[$i]['lng']."', custom : '".$custom."' },"; 
  }else{
	  $buffer_tt .= "{ y: ".$rpt_tt[$i]['t3'].", custom : '".$rpt_tt[$i]['t3']." Lts' },"; 
...
<div id="report">
  <div class="tabs">
    <ul>
      <li class="active"><a href="#" class="onClickTab" rel="tanque_1">Grafica de Combustible</a></li>
      <li><a href="#" class="onClickTab" rel="tanque_2">Detalle de Informaci&oacute;n</a></li>
      <li><a href="#" class="onClickTab" rel="tanque_3">Rendimiento de la Unidad</a></li>
    </ul>
    <br class="clear" />
  </div>
...
options = {
        ...
        xAxis: { 
		  ...
		  labels: { enabled : false, rotation: -90, style: { fontSize: '10px', fontFamily: 'Arial, sans-serif' } },
		  events: {
...
$(function () {
 ...
 dchart = new Highcharts.Chart(doptions);
    ...
    <div id="tanque_2" class="tab">
      ...
      <div id="holder" style="position:relative; overflow-x:scroll; width:98%; margin:auto; height:auto; border:#e4e4e4 solid 1px;"><div id="detail"></div></div>
      <style>
	  div.datagrid { width:98%; margin:auto; position:relative;}
	  div.datagrid .left { width:49%; float:left; border:#000 solid 1px;}
	  div.datagrid .right { width:49%; float:right; border:#000 solid 1px; position:absolute; height:500px; right:0;}
	  .clear { clear:both;}
	  </style>
      <div class="datagrid">
...
<div id="fixed" class="right">
  <div id="map-canvas" style="width:100%; height:500px; display:block; position:relative;"></div>
  <br class="clear" />
</div>
  ...
  <!-- T3 -->
    ...
    <tr bgcolor="#dddddd">
      <td colspan="2">VEHICULO: <?php echo $asset[0]['alias']; ?> (<?php echo $odometer ?> kms)</td>
    </tr>
    <tr>
      <td>Ruta</td>
      <td>Sin Asignar</td>
    ...
    <tr>
      <td>Inicio</td>
      <td><?php echo $objAsset->formatDateTime($rpt[0]['date'],"max"); ?></td>
    </tr>
    <tr>
      <td>Fin</td>
      <td><?php echo $objAsset->formatDateTime($rpt[count($rpt)-1]['date'],"max"); ?></td>
    </tr>
    
    <tr bgcolor="#dddddd">
    <td>Distancia Recorrida Apr&oacute;x.</td>
    </tr>
    <tr>
      <td>Distancia</td>
    ...
    <tr>
      <td>Prendido</td>
      <td><?php echo secondsToTime($ton*60); ?> M</td>
    </tr>
     <tr>
      <td>Apagado</td>
      <td><?php echo secondsToTime($toff*60); ?> M</td>
    </tr>
    ...
    <tr>
      <td>Promedio</td>
      <td><?php echo number_format((array_sum($ttemp) / count($ttemp)),1); ?><?php $tunit = 'C&deg'; if($_SESSION['logged']['temp']=="f"){ $tunit = 'F&deg';  } echo $tunit; ?> </td>
      <td>
        <?php 
		  $aux_a;
		  $aux_b;
    ...
    <tr>
      <td>Maxima</td>
      <td><?php echo max($ttemp);?></td>
      <td>Fecha y Hora</td>
    </tr>
    <tr>
      <td>Minima</td>
      <td><?php echo min($ttemp);?></td>
      <td>Fecha y Hora</td>
    </tr>
    ...
    <tr>
      <td>Promedio</td>
      <td><?php echo number_format((array_sum($tspeed) / count($tspeed)),2); ?></td>
    </tr>
     <tr>
      <td>Maxima</td>
      <td><?php echo max($tspeed);?> km/h</td>
      <td>Fecha y Hora</td>
    </tr>
     ...
     <tr>
    <td bgcolor="#dddddd" colspan="3">Historico de Combustible</td>
    </tr>
    <tr>
      <td>Inicial</td>
      <td><?php echo $rpt_tt[0]['t1']; ?> L</td>
    </tr>
     <tr>
      <td>Final</td>
      <td><?php echo $rpt_tt[count($rpt)-1]['t1']; ?> L</td>
      <td>Fecha y Hora</td>
    </tr>
     <tr>
      <td>Minimo</td>
      <td><?php echo min($tt_raw); ?></td>
      <td>Fecha y Hora</td>
    </tr>
     <tr>
      <td>Maximo</td>
      <td><?php echo max($tt_raw); ?></td>
      <td>Fecha y Hora</td>
    </tr>
    <tr>
      <td>Volumen Recargado</td>
      <td><?php echo $recarga; ?> lts</td>
    </tr>
    
    <tr>
      <td>Volumen Descargado</td>
      <td><?php echo $descarga; ?> lts</td>
    </tr>
	...
	?>
    <tr bgcolor="#dddddd">
    <td>Rendimiento de Unidad</td>
    </tr>
    <tr>
      <td>Combustible Consumido</td>
      <td><?php echo $comb_consumido; ?> lts</td>
    </tr>
     <tr>
      <td>Consumo x 100KM</td>
      <td><?php echo number_format(($comb_consumido/$distance)*100,2); ?> lts</td>
    </tr>
     <tr>
      <td>Rendimiento X LITRO</td>
      <td><?php echo number_format(($distance/$comb_consumido),2); ?> km/lt</td>
    </tr>
     <tr>
      <td>Consumo x Hora</td>
      <td><?php echo number_format(($comb_consumido/($ton/60)),2); ?> lts/hr</td>
    </tr>
