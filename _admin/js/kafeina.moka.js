...
function remove(e){
  var ctrl = $(e.target).parents("tbody").attr("id");
  var id = $(e.target).attr("rel");
  alert(123);
  alert(id);
  if(confirm("Desea eliminar el registro?")){ 
  $.post('_ctrl/ctrl.'+ctrl+'.php', { id: $(e.target).attr("rel"), exec: "delete" }, 
    function(data){
	  console.log(data);
	  $("#tbl_"+ctrl +" > tbody tr").removeClass(); 
	  $(e.target).parent().parent("tr").fadeOut('fast',function(){  $(this).remove(); $("#tbl_"+ctrl +" > tbody tr:odd").addClass("odd"); });  
	}); 
  }
}
